...
{
	int itemUPC;
	char item_name[LIMIT];
	double price;
}database;

void signalHandler(int sig); //smooth release on Ctrl+C
...
void databaseInit(char *file_name)
{
	...
	fgets(line_buff,LIMIT,fp); //reads the first line that has the number of entries in the text file
	records=str_to_int(line_buff); //gets number of records
...
void childprocess(int connSD,int id)
{
	...
	double total=0.0;
	char buffer[LIMIT],msg[2*LIMIT],*token;
	const char delim[2]=" ";
	while(1)
	{
		len=tokenCount=index=quantity=0;
		memset(msg,0,LIMIT); //clears contents of msg
		len=recv(connSD,buffer,LIMIT,0);
		if(len<0)
		{
...
int main(int argc, char *argv[])
{
	...
	ServAddr.sin_port = htons(str_to_int(argv[1])); //self defined server port
	//Binding socket
